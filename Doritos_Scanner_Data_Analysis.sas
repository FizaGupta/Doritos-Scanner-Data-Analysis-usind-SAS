...
where a.iri_key = b.iri_key and Brand = 'DORITOS' ;
quit;
...
proc sql;
create table grocprodloc_f_d_pp as
SELECT IRI_KEY as IRI_KEY, WEEK as WEEK, sum(DOLLARS) as TotalSales, Avg(AvgPPU) as AvgPrice, Avg(DISPLAY)as AvgDisplay, 
Avg(f_plus)as Avg_f_plus, Avg(f_large)as Avg_f_large,
Avg(f_medium)as Avg_f_medium, Avg(f_none)as Avg_f_none, Avg(Price_Reduced) as avg_Price_Reduced
FROM groc_prod_loc_f_d_pp
...
id IRI_KEY WEEK;       
model TotalSales = AvgPrice AvgDisplay Avg_f_plus Avg_f_large Avg_f_none Avg_f_medium Avg_Price_Reduced /  rantwo ; 
run;
...
proc panel data=grocprodloc_f_d_pp plots =None;       
id IRI_KEY WEEK;       
model TotalSales = AvgPrice AvgDisplay Avg_f_plus Avg_f_large Avg_f_none Avg_f_medium Avg_Price_Reduced/  fixtwo ; 
run;
